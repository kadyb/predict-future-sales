...
firstAppearShop = sales[, .(firstAppearShop = min(date_block_num)), 
                        by = .(item_id, shop_id)]
firstAppear$item_id = as.numeric(levels(firstAppear$item_id))[firstAppear$item_id]
col = "firstAppearShop$item_id"
firstAppearShop$item_id = as.numeric(levels(col))[col]
```

8. The number of months the product has been sold generally and in specific stores.

```{r}
salesDuration = sales[, .(salesDuration = max(date_block_num) - min(date_block_num)), 
                      by = .(item_id)]
salesDurationShop = sales[, .(max(date_block_num) - min(date_block_num)), 
                          by = .(item_id, shop_id)]
colnames(salesDurationShop)[3] = "salesDurationShop"
col = "salesDuration$item_id"
salesDuration$item_id = as.numeric(levels(col))[col]
col = "salesDurationShop$item_id"
salesDurationShop$item_id = as.numeric(levels(col))[col]
```
...
salesPrepared[, (intCols) := lapply(.SD, as.numeric.factor), .SDcols = intCols]
...
salesPrepared = merge(salesPrepared, salesDuration, by = "item_id")
salesPrepared = merge(salesPrepared, salesDurationShop, by = c("item_id", "shop_id"))
```
...
ps = ParamSet$new(
  params = list(
    ...
    ParamDbl$new(id = "colsample_bytree",  lower = 0.9, upper = 1),
...
# sprintf("RMSE: %s", round(sqrt(rr$aggregate()), 2))
...
# at$tuning_result$tune_x
# error for these hyperparametrs
# at$tuning_result$perf

# inner loop error
# at$tuning_instance$archive()$regr.rmse
# outer loop error
# rr$score(msr("regr.rmse"))$regr.rmse
```
