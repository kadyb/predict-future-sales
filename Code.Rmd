...
```{r}
# TRZEBA TO POPRAWIC NA WYSPAIENIA UNIKALNE, A NIE WSZYSTKIE!!!
# opis tez do poprawy
catCounts = sales[, .(.N), by = .(item_category_id)][order(-N)]

ggplot(catCounts, aes(N)) +
  geom_histogram(bins = 50) +
  geom_vline(xintercept = median(catCounts[, N]), linetype = "dashed") +
  scale_x_continuous(breaks = c(0, 200000, 400000),
                     labels = c("0", "200000", "400000")) +
  labs(title = "Unique items in categories", 
       caption = "The dashed line represents the median") +
  xlab("Items") +
  ylab("Frequency") +
  theme_light()
```

Category **cinema** (i.e. ID 40) contains over 550 000 items (note that they are not unique), while 12 categories contain less than 20 items. The median for all categories is nearly 7 000 items and the average is around 35 000 items.
...
Most items are sold in the **cinema** category, at the same time it is the most numerous category. The next two categories are **computer games** and **music**.
...
```{r}
# kodowanie znakow nie dziala
sales[item_id == 20949, item_name][1]
```
...
```{r}
# kodowanie znakow nie dziala
sales[item_id == 2808, item_name][1]
```
