...
salesLag = salesLagFun(salesLag, "cor_item_id", 2)
salesLag = salesLag[, -"item_cnt_month"]
```

11. Add new items from the testset to training data and set their sales value to 0. In addition, mark which item is new.

```{r}
newItems = data.table(pair = pairShopItemTest[subsetShopItem])
newItems = CJ(pair = newItems$pair, date_block_num = 0:33)
newItems = newItems[, c("shop_id", "item_id") := tstrsplit(pair, " ", fixed = TRUE)]
newItems = newItems[, -"pair"]
newItems[, item_cnt_month := 0]
newItems[, (intCols) := lapply(.SD, as.integer), .SDcols = intCols]
newItems[, newItemShop := 1]

itemUniqueID = data.table(
  item_id = as.numeric.factor(unique(testset[, item_id])),
  newItem = !(unique(testset[, item_id]) %in% unique(sales[, item_id])) # negation
)
itemUniqueID$newItem = as.numeric(itemUniqueID$newItem)
itemUniqueID = itemUniqueID[newItem == 1, ]
```
...
salesPrepared = merge(salesPrepared, itemUniqueID, by = "item_id", all.x = TRUE)
...
salesPrepared[is.na(sales_lag_2_cor_item_id), sales_lag_2_cor_item_id := 0]
```
...
ps = ParamSet$new(
  params = list(
    ParamDbl$new(id = "eta", lower = 0.05, upper = 0.3),
...
4. Ile razy przez caly czas sprzedazy przedmiot byl zwracany
Czy przedmiot kiedykolwiek byl zwracany (0/1)
...
2. Nie stworzono kombinacji wszystkich przedmiotów w sklepach dla każdego miesiąca, ponieważ liczba wszystkich powtórzeń wynosiła blisko 43 mln.
