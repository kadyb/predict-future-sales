...
ggplot(catCounts, aes(N)) +
  ...
  geom_vline(xintercept = mean(catCounts[, N]), linetype = "dashed") +
  geom_vline(xintercept = median(catCounts[, N])) +
  scale_x_continuous(breaks = c(0, 200000, 400000),
                     labels = c("0", "200000", "400000")) +
  labs(title = "Number of items in categories", 
       caption = "The solid line represents the median
...
Category ID 40 (cinema) contains over 550 000 items, while 12 categories contain less than 20 items. The median for all categories is nearly 7 000 items and the average is around 35 000.

```{r}
catCounts[c(1:5, 80:84)]
```
