...
```{r}
items$cor_items = str_to_lower(items$item_name, locale = "ru")
items$cor_items = sapply(str_split(items$cor_items, "\\("), `[`, 1)
items$cor_items = sapply(str_split(items$cor_items, "\\["), `[`, 1)
items$cor_items = str_replace_all(items$cor_items, "[^A-Za-z0-9А-Яа-я]+", " ")
items$cor_items = str_squish(items$cor_items)
...
10. Item sales in last month by shop
```{r eval = FALSE}
# group_by(shop_id, item_id, date_block_num)
# last_sales = sum(item_cnt_day)
# jesli nie ma sprzedazy, to 0

# powinienem dodac jeszcze dane z trainsetu tutaj

salesLag = sales[, .(item_cnt_month = sum(item_cnt_day)),
                 by = .(shop_id, item_id, date_block_num)]

salesLag = salesLag[salesLag[, .(shop_id, item_id, item_cnt_month,
                                 date_block_num = date_block_num + 1)],
                    on = c("shop_id", "item_id", "date_block_num"),
                    sales_lag_1 := i.item_cnt_month][]
salesLag[is.na(sales_lag_1), "sales_lag_1"] = 0
salesLag[sales_lag_1 < 0, "sales_lag_1"] = 0
salesLag[sales_lag_1 > 20, "sales_lag_1"] = 20

salesLag = salesLag[salesLag[, .(shop_id, item_id, item_cnt_month,
                                 date_block_num = date_block_num + 2)],
                    on = c("shop_id", "item_id", "date_block_num"),
                    sales_lag_2 := i.item_cnt_month][]
salesLag[is.na(sales_lag_2), "sales_lag_2"] = 0
salesLag[sales_lag_2 < 0, "sales_lag_2"] = 0
salesLag[sales_lag_2 > 20, "sales_lag_2"] = 20

salesLag = salesLag[salesLag[, .(shop_id, item_id, item_cnt_month,
                                 date_block_num = date_block_num + 3)],
                    on = c("shop_id", "item_id", "date_block_num"),
                    sales_lag_3 := i.item_cnt_month][]
salesLag[is.na(sales_lag_3), "sales_lag_3"] = 0
salesLag[sales_lag_3 < 0, "sales_lag_3"] = 0
salesLag[sales_lag_3 > 20, "sales_lag_3"] = 20
```
...
salesPrepared = merge(salesPrepared, firstAppear, by = "item_id")
salesPrepared = merge(salesPrepared, firstAppearShop, by = c("item_id", "shop_id"))
salesPrepared = merge(salesPrepared, salesDur, by = "item_id")
salesPrepared = merge(salesPrepared, salesDurShop, by = c("item_id", "shop_id"))
salesPrepared = merge(salesPrepared, numItemShop, by = c("shop_id"))
```

Merge main datatable with created explanatory variables.
...
# NA for items and (items, shop) which don't appear in sales datatable
testset = merge(testset, firstAppear, by = "item_id", all.x = TRUE)
testset[is.na(testset$firstAppear), "firstAppear"] = 0
testset = merge(testset, firstAppearShop, by = c("item_id", "shop_id"), all.x = TRUE)
testset[is.na(testset$firstAppearShop), "firstAppearShop"] = 0
...
ps = ParamSet$new(
  params = list(
    ParamDbl$new(id = "eta", lower = 0.05, upper = 0.4),
    ...
    ParamDbl$new(id = "colsample_bytree",  lower = 0.8, upper = 1),
    ParamDbl$new(id = "colsample_bylevel", lower = 0.5, upper = 1),
...
results = as.data.table(at$predict_newdata(task, testset))$response
results[results < 0] = 0
...
9. pewnie bedzie trzeba dodac dane z testsetu do trainsetu i ustawic ich wartosci na 0 - zrobic test i porownac. Albo zamiast 0 ustawic mediany
10. dodac zmienna logiczna:
a) czy produkt byl w trainset
b) czy tuplet (sklep, produkt) byl w trainset
11. stworzyc komibnacji wszystkich mozliwych sklepow, przedmiotow i miesiecy, nastepnie przypisac wartosci liczby sprzedazy = 0, tym ktore nie pojawily sie w zbiorze treningowym(??????)


Czego nie zrobiono:

1. Pominięto generowanie nowych ID przedmiotów na podstawie podobieństw między nazwami (biblioteka "stringdist"), ponieważ macierz wynikowa składała się z 22170^2 obserwacji.
