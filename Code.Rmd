...
title = ggdraw() + draw_label("Data aggregated by categories")
...
pg = plot_grid(p1, p2, p3, ncol = 3)
title = ggdraw() + draw_label("Data aggregated by shops")
plot_grid(title, pg, ncol = 1, rel_heights = c(0.1, 1))
```
...
```{r}
month = rep(1:12, 3)
year = c(rep(2013, 12), rep(2014, 12), rep(2015, 12))
...
shopLabels = data.table(shop_id = shops$shop_id, shopLabel = as.numeric(shopLoc))
```
...
```{r}
# Aggregate input data to main datatable
...
salesPrepared[, (intCols) := lapply(.SD, as.numeric), .SDcols = intCols]
```
...
9. pewnie bedzie trzeba dodac dane z testsetu do trainsetu i ustawic ich wartosci na 0 - zrobic test i porownac
10. pamietac zeby przyciac wartosci do zakresu 0-20 (wieksza niz 20 staja sie 20) - zrobic to przed trenowaniem (raczej) czy po trenowaniu?
11. czy sklep interentowy wyznaczyc jako osobny sklep (sprzedaje tych przedmioty cyfrowe)?
